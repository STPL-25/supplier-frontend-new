          ...
          {datas.length > 0 ? (
            <>
              ...
              <div className="m-auto mt-5 text-center">
                ...
                ) : userRole === "Admin" || userRole === "MIS" || userRole === "Demo-User" ? (
                  <>
                    <button
...
export default DemoKycAprovalMis;
...
// // NEW: Utility function to replace URL
// const replaceApiUrl = (url) => {
//   if (typeof url === 'string' && url.includes('https://advback.spacetextiles.net')) {
//     return url.replace('https://advback.spacetextiles.net', 'http://10.0.20.4:8080');
//   }
//   return url;
// };
...
//   // NEW: Replace URLs from context
//   const kycApiUrl = replaceApiUrl(KYC_API);
//   const apiUrl = replaceApiUrl(API);
  
//   console.log("user", user);
//   console.log("Original KYC_API:", KYC_API);
//   console.log("Replaced kycApiUrl:", kycApiUrl);
...
//         const encodedUserRole = btoa(userRole);
//         const response = await axios.get(
//           `${kycApiUrl}/gettingkycdetails/${supplierDetails}/${encodedUserRole}`
//         );
//         console.log(response.data);
//         setDatas(response.data);
//         if (response.data.length > 0) {
...
//   const fetchSupplierNames = async (userRole) => {
//     try {
//       const response = await axios.post(`${kycApiUrl}/getsuppliernames`, {
//         userRole: userRole,
//       });
//       return response.data;
//     } catch (error) {
...
//     // NEW: Replace URL in file paths as well
//     const replacedUrl = replaceApiUrl(cleanUrl);

//     const fileType = replacedUrl.toLowerCase().endsWith(".pdf") ? "pdf" : "image";

//     setCurrentFileUrl(replacedUrl);
//     setCurrentFileType(fileType);
//     setFileViewerOpen(true);
//     setPageNumber(1);
//     setScale(1.0);
...
//       setIsRejected(false);
//       const response = await axios.put(
//         `${kycApiUrl}/kycdatachecking/${supplierId}/${action}/${user}`
//       );
//       if (response.status === 200) {
//         setSuccessMsg(`${action}ed successfully.`);
//         setSnackbarSeverity("success");
//         setOpenSnackbar(true);
//         if (action === "Approve") {
//           const res = await axios.get(`${kycApiUrl}/approved/sms/${supplierId}`);
//           console.log(res.data);
//         }
//         setSupplierDetails("");
//         setDatas([]);
...
//       const response = await axios.put(
//         `${kycApiUrl}/kycdatachecking/${supplierId}/Reject/${user}`,
//         { reason: rejectReason, RejectectedBy: encodedUserRole }
//       );
//       if (response.status === 200) {
//         const res = await axios.get(`${kycApiUrl}/rejected/sms/${supplierId}`);
//         console.log(res.data);
//         setSuccessMsg("Rejected successfully.");
//         setSnackbarSeverity("success");
//         setOpenSnackbar(true);
