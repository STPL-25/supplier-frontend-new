...
const DiamondPDF = ({ data }) => {
  ...
  const hasGoldData = data.some(row => 
    ...
    (row.GoldPurity && row.GoldPurity.toString().trim() !== '')
  );
  ...
  const hasPlatinumData = data.some(row => 
    ...
    (row.PTPurity && row.PTPurity.toString().trim() !== '')
  );
  ...
  const hasWastageData = data.some(row => row.WastageAmt && Number(row.WastageAmt) > 0);
  const hasGoldMakingData = data.some(row => row.GoMcAmount && Number(row.GoMcAmount) > 0);
  const hasPtMakingData = data.some(row => row.PTMcAmount && Number(row.PTMcAmount) > 0);
  const hasHandleChargeData = data.some(row => row.HandleAmount && Number(row.HandleAmount) > 0);
  const hasHMData = data.some(row => row.HMTaxableAmt && Number(row.HMTaxableAmt) > 0);
  const hasCertData = data.some(row => row.CertTaxableAmt && Number(row.CertTaxableAmt) > 0);

  // Calculate dynamic widths
  const baseColumns = 2;
  const metalColumns = (hasGoldData ? 1 : 0) + (hasPlatinumData ? 1 : 0);
  const diamondColumns = hasDiamondData ? 5 : 0;
  const colorStoneColumns = hasColorStoneData ? 5 : 0;
  ...
  const metalSectionWidth = metalColumns > 0 ? Math.max(baseWidth * 4, 16) : 0;
  ...
  const SubHeader = ({ width, children }) => (
    <Text style={[pdfStyles.columnHeader, { width: `${width}%` }]}>{children}</Text>
  );
  ...
  const TotalCell = ({ width, children }) => (
    <Text style={[pdfStyles.totalCell, { width: `${width}%` }]}>{children}</Text>
  );

  const GroupHeader = ({ width, children }) => (
    <Text style={[pdfStyles.groupHeader, { width: `${width}%` }]}>{children}</Text>
  );
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={pdfStyles.page}>
        <Text style={pdfStyles.header}>Diamond Dealer Receipt</Text>
        ...
        <View style={pdfStyles.table}>
          ...
          {/* Data Rows */}
          {data.map((row, index) => (
            <View key={index} style={[pdfStyles.tableRow, index % 2 === 1 && pdfStyles.alternateRow]}>
          ...
          {/* Totals Row */}
          <View style={[pdfStyles.tableRow, pdfStyles.totalsRow]}>
