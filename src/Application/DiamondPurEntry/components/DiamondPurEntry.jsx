...
const getPoNumberValue = po => {
  if (typeof po === 'string') return normalizeText(po);
  if (!po || typeof po !== 'object') return '';
  return normalizeText(po.poNumber ?? po.PONumber ?? po.po_number ?? '');
};

const getSupplierNameValue = po => {
  if (!po || typeof po !== 'object') return '';
  return normalizeText(po.supplierName ?? po.SupplierName ?? po.supplier_name ?? '');
};
...
const POSelectorPanel = ({
  selectedSupplier, supplierOptions, selectedPO, poDetails, poNumbers, poLoading, poError,
  onSelectSupplier, onSelectPO, poDetailLoading,
}) => (
    ...
    <div className="flex items-center gap-3 flex-wrap">
      ...
      <div className="relative min-w-[220px]">
        <select
          ...
          disabled={poLoading || (supplierOptions.length > 0 && !selectedSupplier)}
          className="w-full appearance-none pl-3 pr-8 py-2 border border-gray-300 rounded-lg text-sm
                     focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-white shadow-sm
                     disabled:bg-gray-100 disabled:cursor-not-allowed"
        >
...
// console.log(suppCode)
  ...
  const filteredPoNumbers = useMemo(() => (
    poNumbers
      .filter(po => !selectedSupplier || getSupplierNameValue(po) === selectedSupplier)
      .map(getPoNumberValue)
      .filter(Boolean)
  ), [poNumbers, selectedSupplier]);
  ...
  const handleSelectPO = async (poNumber) => {
    const selectedPoMeta = poNumbers.find(po => getPoNumberValue(po) === poNumber);
    const poSupplierName = getSupplierNameValue(selectedPoMeta);
   ...
   const res = await axios.post(`${API}/gold_po/fetch_po_creation`, {
        selectedSupplier: poSupplierName || selectedSupplier || suppliers?.companyname || user,
      ...
      setPoDetails({
        ...
        supplierName:     supplierAddr.name           || firstItem.supplierName || poSupplierName || selectedSupplier || '',
      ...
      setPoDetails({ poNumber, supplierName: poSupplierName || selectedSupplier || '' });
    } finally {
      ...
      {selectedPO && poDetails && !poDetailLoading && (
        <div className="flex items-center gap-3 flex-wrap bg-white border rounded-lg p-4 shadow-sm">
          ...
          <TemplateDownload
            ...
            supplierCode={poDetails.supplierCode  || suppliers?.Suppcode    || suppCode || ''}
            poNumber={selectedPO}
            poDetails={poDetails}
...
export default DiamondPurEntry;
