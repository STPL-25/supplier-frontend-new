...
// disabled = true  → only shows the badge, no dropdown arrow
...
// ─── Reason Constants ────────────────────────────────────────
const PASS_REASONS = ['All items verified', 'Weight matches', 'Hallmark OK', 'Stone setting OK'];
const REJECT_REASONS = ['Surface cracks', 'Weight mismatch', 'Missing hallmark', 'Stone setting issue', 'Finishing defect', 'Wrong design'];

// ─── Multi-Select Reason Chips ───────────────────────────────
const MultiReasonSelect = ({ mode, selected = [], onChange }) => {
  const options     = mode === 'pass' ? PASS_REASONS : REJECT_REASONS;
  const colorActive = mode === 'pass' ? 'bg-green-500 text-white border-green-500' : 'bg-red-500 text-white border-red-500';
  const colorHover  = mode === 'pass' ? 'hover:border-green-400 hover:text-green-600' : 'hover:border-red-400 hover:text-red-600';
      ...
      {options.map((r) => (
        <button
          ...
          className={`px-2 py-1 text-xs rounded-full border transition-colors ${
            selected.includes(r) ? colorActive : `bg-white text-gray-600 border-gray-300 ${colorHover}`
          }`}
        >
          {r}
        </button>
...
const QCCheckReport = () => {
  ...
  // ── Helper: is this row still actionable (Pending/Partial) ──
  // ✅ KEY FIX: read directly from qcData, fallback to item status
  const getRowStatus = (id) => {
    return (qcData[id] || {}).status || 'Pending';
  };
  ...
  // List of row IDs that are still actionable (not yet Approved/Rejected)
  const pendingIds = filteredData.map((i) => getRowId(i)).filter(isPending);

  // ✅ FIX: allSelected checks if every pending ID is inside selectedIds
  const allSelected  = pendingIds.length > 0 && pendingIds.every((id) => selectedIds.has(id));
  // ✅ FIX: someSelected = at least one selected but not all pending selected
  const someSelected = selectedIds.size > 0 && !allSelected;

  const toggleSelect = (id) => {
    if (!isPending(id)) return; // block Approved/Rejected rows
    setSelectedIds((prev) => {
  ...
  // ✅ FIX: select/deselect only pendingIds, leave Approved/Rejected untouched
  const toggleSelectAll = () => {
    if (allSelected) {
      // deselect all pending rows
      setSelectedIds((prev) => {
        const next = new Set(prev);
      ...
      // select all pending rows (keep any existing non-pending selections — none should exist, but safe)
      setSelectedIds((prev) => {
        const next = new Set(prev);
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-gray-50 border-b sticky top-0 z-10">
                  <tr>
                    <th className="px-3 py-3 text-center w-10">
                      {/* ✅ Header checkbox: only controls pending rows */}
                      <input
                        type="checkbox"
                        checked={allSelected}
                ...
                <tbody>
                  {filteredData.map((item, index) => {
                    ...
                    // ✅ isProcessed: row is locked once Approved or Rejected
                    ...
                    return (
                      ...
                      >
                        <td className="px-3 py-3 text-center">
                          {/* ✅ Hide checkbox for Approved/Rejected rows */}
                          {isProcessed ? (
                        ...
                        <td className="px-3 py-3 text-center">
                          {/* ✅ No dropdown for processed rows */}
                          <QCStatusDropdown
      ...
      {qcActionModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-60 w-full max-w-md bg-white rounded-xl shadow-2xl overflow-hidden">
            <div className={`p-4 border-b flex justify-between items-center text-white ${qcActionModal.mode === 'pass' ? 'bg-gradient-to-r from-green-500 to-emerald-600' : 'bg-gradient-to-r from-red-500 to-rose-600'}`}>
              <h3 className="text-lg font-bold flex items-center gap-2">
                {qcActionModal.mode === 'pass' ? <><CheckCircle className="w-5 h-5" /> QC Pass</> : <><XCircle className="w-5 h-5" /> Reject</>}
                {qcActionModal.ids.length > 1 && <span className="ml-1 text-sm font-normal opacity-80">({qcActionModal.ids.length} items)</span>}
              </h3>
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-gray-600">
                {qcActionModal.mode === 'pass' ? (
                  <>Marking <strong>{qcActionModal.ids.length}</strong> item(s) as <span className="text-green-600 font-semibold">QC Passed</span>. Select notes (optional).</>
                ) : (
                  <><span className="text-red-600 font-semibold">Rejecting</span> <strong>{qcActionModal.ids.length}</strong> item(s). Select reason(s) — <strong>required</strong>.</>
                )}
              </p>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  {qcActionModal.mode === 'pass' ? 'Pass Notes' : 'Rejection Reasons'}
                  {qcActionModal.mode === 'reject' && <span className="text-red-500 ml-1">*</span>}
                  <span className="text-gray-400 text-xs ml-1">(select multiple)</span>
                </label>
                <MultiReasonSelect mode={qcActionModal.mode} selected={qcChips} onChange={setQcChips} />
              </div>
              {qcChips.length > 0 && (
                <div className="px-3 py-2 bg-gray-50 rounded-lg border text-xs text-gray-600 break-words">
                  <span className="font-semibold text-gray-500">Selected: </span>{qcChips.join(', ')}
                </div>
              )}
              <div>
                ...
                <textarea
                  ...
                  placeholder={qcActionModal.mode === 'pass' ? 'e.g. All items verified...' : 'e.g. Batch rejected due to supplier error...'}
                  className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 text-sm resize-none ${qcActionModal.mode === 'pass' ? 'focus:ring-green-400' : 'border-red-200 focus:ring-red-400'}`}
                />
              </div>
            </div>
            <div className="p-4 border-t bg-gray-50 flex justify-end gap-3">
              <button onClick={closeQCModal} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 text-sm">
                Cancel
              </button>
              <button
                onClick={saveQCAction}
                disabled={submitting || (qcActionModal.mode === 'reject' && qcChips.length === 0)}
                className={`px-5 py-2 text-white rounded-lg flex items-center gap-2 text-sm disabled:opacity-60 ${qcActionModal.mode === 'pass' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}`}
              >
                {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : qcActionModal.mode === 'pass' ? <CheckCircle className="w-4 h-4" /> : <XCircle className="w-4 h-4" />}
...
// // Props:
// //   mode      : 'pass' | 'reject'
// //   selected  : string[]   ← array of selected chip labels
// //   onChange  : (string[]) => void
// // ─────────────────────────────────────────────────────────────
// const MultiReasonSelect = ({ mode, selected = [], onChange }) => {
//   const options     = mode === 'pass' ? PASS_REASONS : REJECT_REASONS;
//   const colorActive = mode === 'pass'
//     ? 'bg-green-500 text-white border-green-500'
//     : 'bg-red-500 text-white border-red-500';
//   const colorHover = mode === 'pass'
...
//             selected.includes(r)
//               ? colorActive
//               : `bg-white text-gray-600 border-gray-300 ${colorHover}`
//           }`}
//         >
//           {r}
//         </button>
...
//   // ── QC Modal State ────────────────────────────────────────
//   // qcChips    : string[]  — selected reason chip labels
//   // qcNote     : string    — free-text additional note
//   const [qcActionModal, setQcActionModal] = useState({ open: false, mode: 'pass', ids: [] });
//   const [qcChips, setQcChips]             = useState([]);   // ← was: qcReason (string)
//   const [qcNote, setQcNote]               = useState('');   // ← was: hardcoded '' textarea
...
//   // Build a structured reason payload from chips + free-text note
//   // Returns: { reasons: string[], note: string, display: string }
//   const buildReasonPayload = () => {
//     const reasons = [...qcChips];
//     const note    = qcNote.trim();
//     const display = note ? [...reasons, note].join(', ') : reasons.join(', ');
...
//   // ── QC Action ─────────────────────────────────────────────
//   const openQCAction = (mode, ids) => {
//     setQcChips([]);   // reset chip selections
//     setQcNote('');    // reset free-text note
//     setQcActionModal({ open: true, mode, ids });
//   };
...
//   // Reject requires at least one chip reason selected
//   if (mode === 'reject' && reasons.length === 0) {
//     alert('Please select at least one rejection reason.');
//     return;
//   }
...
//   setSubmitting(true);
//   try {
//     // ── Step 1: Build local state updates ──────────────────
//     const updates = {};
//     ids.forEach((id) => {
//       const item = purchaseData.find((p) => getRowId(p) === id);
//       const pcs  = item?.PCS || 0;
...
//     // ── Step 2: Build a single array of objects for the API ─
//    const payload = ids.map((id) => ({
//   purchaseId:  id,
//   approvedQty: updates[id].approvedQty,
//   rejectedQty: updates[id].rejectedQty,
//   reasons:     Array.isArray(updates[id].reasons)
//                  ? updates[id].reasons                        // already string[] → use as-is
//                  : (updates[id].reasons || '').split(',').map(s => s.trim()).filter(Boolean), // fallback if somehow a string
//   note:        updates[id].note,
//   status:      updates[id].status,
//   updatedBy:   user,
// }));
...
//     // ── Step 3: Single API call with the full array ─────────
//     const response = await axios.post(`${DIA_API}/saveQCCheck`, { items: payload });
//     console.log(response )
...
//                     <path d="M3 5h2M3 10h2M3 15h2M3 19h2M7 5v14M17 5v14M21 5h-2M21 10h-2M21 15h-2M21 19h-2M12 5v14" strokeLinecap="round" />
//                   </svg>
//                   Scan or type
//                 </span>
//               </label>
...
//                       <tr
//                         key={id}
//                         className={`border-b hover:bg-gray-50 transition-colors ${rowBg} ${
//                           isSelected ? 'ring-2 ring-inset ring-indigo-300' : ''
//                         }`}
//                       >
//                         <td className="px-3 py-3 text-center">
//                           <input
...
//               {qcChips.length > 0 && (
//                 <div className="px-3 py-2 bg-gray-50 rounded-lg border text-xs text-gray-600 break-words">
//                   <span className="font-semibold text-gray-500">Selected: </span>
//                   {qcChips.join(', ')}
//                 </div>
//               )}
...
//                   placeholder={
//                     qcActionModal.mode === 'pass'
//                       ? 'e.g. All items verified and passed...'
...
//                 disabled={
//                   submitting ||
//                   (qcActionModal.mode === 'reject' && qcChips.length === 0)
//                 }
...
//                   {selectedEntry.SupplierName} — {selectedEntry.invoiceNumber}
//                 </p>
//               </div>
//               <button onClick={() => setShowDetailModal(false)} className="p-2 hover:bg-white/20 rounded-full">
//                 <X className="w-5 h-5" />
...
//                   <div>
//                     <div className="text-xs text-gray-500 font-medium">Gross Wt (g)</div>
//                     <div className="text-[10px] text-gray-400 mb-1">GoldWt + PlatinumWt + ColorStoneWt + DiamondWt</div>
//                     <div className="p-2 bg-amber-50 rounded border font-bold text-lg text-amber-700">
//                       {fmt3(getGrossWt(selectedEntry))}
//                     </div>
//                     <div className="mt-1 grid grid-cols-2 gap-1 text-[10px] text-gray-500">
//                       <span>Gold: {fmt3(selectedEntry.GoldWt)}</span>
//                       <span>Platinum: {fmt3(selectedEntry.PlatinumWt)}</span>
...
//                   <div>
//                     <div className="text-xs text-gray-500 font-medium">Cent Wt (ct)</div>
//                     <div className="text-[10px] text-gray-400 mb-1">Σ diamonds.Carat</div>
...
//                   <div>
//                     <div className="text-xs text-gray-500 font-medium">Net Wt (g)</div>
//                     <div className="text-[10px] text-gray-400 mb-1">GoldWt + PlatinumWt (metal only)</div>
//                     <div className="p-2 bg-indigo-50 rounded border font-bold text-lg text-indigo-700">
//                       {fmt3(getNetWt(selectedEntry))}
//                     </div>
//                   </div>
//                 </div>
//               </section>
...
//                       <div>
//                         <div className="text-xs text-gray-500">Approved Qty</div>
//                         <div className="mt-1 p-2 bg-green-50 rounded border font-bold text-green-700 text-lg">
//                           {qc.approvedQty ?? '-'}
//                         </div>
//                       </div>
//                       <div>
//                         <div className="text-xs text-gray-500">Rejected Qty</div>
//                         <div className="mt-1 p-2 bg-red-50 rounded border font-bold text-red-700 text-lg">
//                           {qc.rejectedQty ?? '-'}
//                         </div>
//                       </div>
//                       <div>
...
//                           {qc.display || selectedEntry.RejectionReason || '—'}
//                         </div>
//                       </div>
//                     </div>
//                   );
...
//                           <tr>
//                             {['Shape', 'Stones', 'Carat', 'Weight (g)', 'Rate (₹)', 'Value (₹)'].map((h) => (
//                               <th key={h} className="px-3 py-2 text-right border first:text-left font-semibold">
//                                 {h}
//                               </th>
//                             ))}
//                           </tr>
