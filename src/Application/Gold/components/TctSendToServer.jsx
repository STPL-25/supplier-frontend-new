    ...
    <Page size="A4" style={styles.page}>
      ...
      {pageNumber === 1 && (
        <>
          ...
          >
            <QRCodeComponent
              value={parentPoNumber ?? (poAddressData?.poDetails?.poNumber ? `IV${poAddressData.poDetails.poNumber}` : "Unknown PO")}
        ...
        <View style={styles.cardContainer}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.sectionContent}>
              <Text style={styles.label}>
                PO Number{" "}
                <Text style={styles.value}>
                  {parentPoNumber||`IV${poAddressData?.poDetails?.poNumber}` || ""}
                </Text>{" "}
                PO Date{" "}
                <Text style={styles.value}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.sectionContent}>
              ...
              <Text style={styles.label}>
                Payment Type{" "}
                {console.log(poAddressData?.delivery,"poAddressData?.delivery")}
                <Text style={styles.value}>
                  {(() => {
                    try {
                      if (poAddressData?.delivery) {
                        ...
                        return deliveryData?.paymentType||deliveryData?.payment_Type ||deliveryData?.paymentType|| "IV";
                      }
                      return "IV";
                    } catch (error) {
...
const PODocument = ({ submittedData = [], poAddressData, orderTypes,parentPoNumber }) => {
...
export const useTctSendToServer = ({
  ...
  const generatePdf = async (
   ...
   const parentPoNumber=`${filteredItems[0].ParentCompany.charAt(0)??""}IV${poData?.poDetails?.poNumber ?? ""}`
      ...
      formData.append("poNumber", parentPoNumber);
      formData.append('orignialPoNumber',poData?.poDetails?.poNumber ?? "")
