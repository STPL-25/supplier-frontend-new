...
function SubmittedDataComp({
  ...
  // ✅ Helper to compute totals from a data array
  const computeTotals = (data) => ({
    pieces: data.reduce((sum, item) => sum + (parseFloat(item.pieces) || 0), 0),
    gross_weight: data.reduce((sum, item) => sum + (parseFloat(item.gross_weight) || 0), 0),
    diamondwt: data.reduce((sum, item) => sum + (parseFloat(item.diamondwt) || 0), 0),
    stone_weight: data.reduce((sum, item) => sum + (parseFloat(item.stone_weight) || 0), 0),
    wax_weight: data.reduce((sum, item) => sum + (parseFloat(item.wax_weight) || 0), 0),
    net_weight: data.reduce((sum, item) => sum + (parseFloat(item.net_weight) || 0), 0),
    pure_wt: data.reduce((sum, item) => sum + (parseFloat(item.pure_wt) || 0), 0),
  });
        ...
        <div className="px-4 py-4">
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-3 rounded-lg border border-blue-200">
              <div className="flex items-center gap-2 mb-1">
                <Package className="text-blue-600" size={16} />
                <span className="text-xs font-semibold text-blue-700 uppercase">Pieces</span>
              </div>
              <p className="text-xl font-bold text-blue-900">{item.pieces || 0}</p>
            </div>
            <div className="bg-gradient-to-br from-purple-50 to-purple-100 p-3 rounded-lg border border-purple-200">
              <div className="flex items-center gap-2 mb-1">
                <Weight className="text-purple-600" size={16} />
                <span className="text-xs font-semibold text-purple-700 uppercase">Gross Wt</span>
              </div>
              <p className="text-xl font-bold text-purple-900">
                {item.gross_weight || 0}<span className="text-sm ml-1">g</span>
              </p>
            </div>
            <div className="bg-gradient-to-br from-green-50 to-green-100 p-3 rounded-lg border border-green-200">
              <div className="flex items-center gap-2 mb-1">
                <Weight className="text-green-600" size={16} />
                <span className="text-xs font-semibold text-green-700 uppercase">Net Wt</span>
              </div>
              <p className="text-xl font-bold text-green-900">
                {item.net_weight || 0}<span className="text-sm ml-1">g</span>
              </p>
            </div>
            <div className="bg-gradient-to-br from-amber-50 to-amber-100 p-3 rounded-lg border border-amber-200">
              <div className="flex items-center gap-2 mb-1">
                <DollarSign className="text-amber-600" size={16} />
                <span className="text-xs font-semibold text-amber-700 uppercase">Amount</span>
              </div>
              <p className="text-xl font-bold text-amber-900">₹{item.amount || 0}</p>
            </div>
          </div>
  ...
  const renderMobileTotals = (data) => {
    const totals = computeTotals(data);
    return (
      <div className="bg-gradient-to-r from-indigo-600 to-purple-700 mx-0 p-4 mt-2 rounded-b-2xl shadow-lg">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            { label: "Gross Wt", value: `${totals.gross_weight.toFixed(3)}g` },
            { label: "Diamond", value: `${totals.diamondwt.toFixed(3)}ct` },
            { label: "Stone Wt", value: `${totals.stone_weight.toFixed(3)}g` },
            { label: "Wax Wt", value: `${totals.wax_weight.toFixed(3)}g` },
            { label: "Net Wt", value: `${totals.net_weight.toFixed(3)}g` },
            { label: "Pure Wt", value: `${totals.pure_wt.toFixed(3)}g` },
          ].map(({ label, value }) => (
  ...
  const renderTableContent = (data, title) => {
    ...
    // ✅ Compute totals for this section
    const totals = computeTotals(data);
      ...
      <div className="md:mt-8">
        ...
        <div className="hidden md:block bg-white rounded-b-2xl shadow-xl overflow-hidden">
          ...
          {selectedItems && (
            <div className="flex items-center justify-center p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-blue-100">
              <div className="flex items-center gap-3 bg-white px-6 py-3 rounded-full shadow-md">
                <span className="text-gray-700 font-semibold">Total Selected:</span>
                <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold px-4 py-1 rounded-full">
                  {selectedItems?.length || 0}
                </span>
              </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-gray-100 to-gray-200">
                  ...
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-left font-bold text-gray-700">Product</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-center font-bold text-gray-700">Pieces</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Rate</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Gross Wt</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700 whitespace-nowrap">Diamond (ct)</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Stone Wt</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Wax Wt</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Net Wt</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Amount</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Melting</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Wastage</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">MC</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-right font-bold text-gray-700">Pure Wt</th>
                  <th className="px-4 py-4 border-b-2 border-gray-300 text-center font-bold text-gray-700">Photo</th>
                  {selectedItems && (
                    <th className="px-4 py-4 border-b-2 border-gray-300 text-center font-bold text-gray-700">Actions</th>
                  )}
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <React.Fragment key={item.sno}>
                      <tr className={`${index % 2 === 0 ? "bg-white" : "bg-gray-50"} hover:bg-blue-50 transition-colors duration-150`}>
                        ...
                        <td className="px-4 py-4 border-b border-gray-200 font-semibold text-gray-800">{item.product_name || "NA"}</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-center font-semibold text-blue-600">{item.pieces || 0}</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">₹{item.rate || 0}</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">{item.gross_weight || 0}g</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">{item.diamondwt || 0}ct</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">{item.stone_weight || 0}g</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">{item.wax_weight || 0}g</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right font-semibold text-green-600">{item.net_weight || 0}g</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right font-bold text-amber-600">₹{item.amount || 0}</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">{item.melting || 0}%</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">{item.wastage || 0}%</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right text-gray-700">₹{item.makingCharges || 0}</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-right font-semibold text-purple-600">{item.pure_wt || 0}g</td>
                        <td className="px-4 py-4 border-b border-gray-200 text-center">
                          {item.product_image ? (
                            <img src={item.product_image} alt="Product" className="w-20 h-20 object-cover rounded-lg shadow-md mx-auto hover:scale-110 transition-transform duration-200 cursor-pointer" />
                          ) : (
                            <span className="text-gray-400">No Image</span>
                          )}
                        </td>
                        {selectedItems && (
                          <td className="px-4 py-4 border-b border-gray-200 text-center">
                            <div className="flex justify-center gap-2">
                              <button onClick={() => onEdit(item)} className="p-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-all duration-200 hover:scale-110 shadow-md" title="Edit">
                                <Edit size={18} />
                              </button>
                              <button onClick={() => setActiveModal(item.sno)} className="p-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-all duration-200 hover:scale-110 shadow-md" title="Delete">
                                <Trash2 size={18} />
                              </button>
                            </div>
                          </td>
                ...
                {/* ✅ TOTALS ROW */}
                <tr className="bg-gradient-to-r from-indigo-50 to-purple-50 border-t-2 border-indigo-300 font-bold">
                  {selectedItems && <td className="px-4 py-4" />}
                  {ispocreation && <><td className="px-4 py-4" /><td className="px-4 py-4" /></>}
                  <td className="px-4 py-4 text-indigo-800 font-extrabold text-sm uppercase tracking-wide">
                    Totals
                  </td>
                  <td className="px-4 py-4 text-center text-indigo-700 font-extrabold">
                    {totals.pieces}
                  </td>
                  <td className="px-4 py-4 text-right text-gray-400 text-xs italic">—</td>
                  <td className="px-4 py-4 text-right text-purple-700 font-extrabold">
                    {totals.gross_weight.toFixed(3)}g
                  </td>
                  <td className="px-4 py-4 text-right text-gray-700 font-extrabold">
                    {totals.diamondwt.toFixed(3)}ct
                  </td>
                  <td className="px-4 py-4 text-right text-gray-700 font-extrabold">
                    {totals.stone_weight.toFixed(3)}g
                  </td>
                  <td className="px-4 py-4 text-right text-gray-700 font-extrabold">
                    {totals.wax_weight.toFixed(3)}g
                  </td>
                  <td className="px-4 py-4 text-right text-green-700 font-extrabold">
                    {totals.net_weight.toFixed(3)}g
                  </td>
                  <td className="px-4 py-4 text-right text-gray-400 text-xs italic">—</td>
                  <td className="px-4 py-4 text-right text-gray-400 text-xs italic">—</td>
                  <td className="px-4 py-4 text-right text-gray-400 text-xs italic">—</td>
  ...
  return (
    <div className="w-full md:p-4 bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
      {fairData?.length > 0 && renderTableContent(fairData, "Exhibitions Selection")}
      {officeData?.length > 0 && renderTableContent(officeData, "Office Selection")}
      ...
      {submittedData.length === 0 && (
        <div className="mt-8 text-center p-12 bg-white border-2 border-dashed border-gray-300 md:rounded-2xl shadow-lg mx-4 md:mx-0">
          ...
          <p className="text-gray-600 text-lg font-medium">No data available at the moment.</p>
          <p className="text-gray-400 text-sm mt-2">Items will appear here once added.</p>
        </div>
      )}
    </div>
  );
