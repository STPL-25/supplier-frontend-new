...
// import {
//   FileSpreadsheet, Eye, Download, Upload, AlertCircle, CheckCircle, Save, RefreshCw, Loader2
// } from 'lucide-react';
...
//   'Platinum Mc Type': 'PLTMcType',
//   'Platinum Mc Rate': 'PLTMcRate',
//   'Platinum Mc Amount': 'PLTMcAmount',
...
// // Gold-specific columns to exclude from Platinum template
// const GOLD_COLUMNS = [
//   'Sno', 'G Karat', 'Gold Wt', 'Gold Purity', 'Gold Purity Wt', 
//   'Gold999Rate (without GST)', 'Gold Value', 'Gold Mc Type', 
//   'Gold Mc Rate', 'Gold Mc Amount', 'Gold Wastage Type', 
//   'Gold Wastage Weight', 'Gold Wastage Amount', 'Gold HallMark Type',
//   'Gold HallMark GST', 'GoldHallMark Qty', 'Gold HallMark Rate',
//   'Gold HallMark Taxable Amount', 'Gold HallMark Total', 'Gold Net Wt'
// ];

// // Purchase entry columns for display (backend field names)
// const PURCHASE_COLUMNS = [
//   'id', 'SupplierName', 'ProductName', 'MetalType', 'WtMode', 'DesignNo', 'GCarat', 'PCS', 'GoldWt', 'GoldPurity', 
//   'GoldPurityWt', 'Gold999Rate', 'GoldValue', 'PTWt', 'PTPurity', 'PTPurityWt', 'PT999Rate', 'PTValue',
//   'NoofStone', 'DCarat', 'DiamondRate', 'DiamondValue', 'DiamondWt',
//   'ClrStnPCS', 'CLSCarat', 'ClrStnWt', 'ClrStnRate', 'CSAmount', 'GoMcType', 'GoMcRate', 'GoMcAmount', 
//   'PLTMcType', 'PLTMcRate', 'PLTMcAmount', 'WastageType', 'WastageWeight', 'WastageAmt',
//   'CertType', 'CertGST', 'CertQty', 'CertRate', 'CertTaxableAmt', 'CertTotal',
//   'HallMarkType', 'HMGST', 'HMQty', 'HMRate', 'HMTaxableAmt', 'HMTotal', 'HandleRate', 'HandleAmount',
//   'GNetWt', 'PNetWt', 'TotalValue', 'GST', 'GrandTotal', 'HUID', 'invoiceNumber', 'suppCode'
// ];
...
// // Helper: Map Excel headers (Display Names) to backend field names
// const mapExcelHeadersToBackend = (excelRow) => {
//   const mappedRow = {};
//   Object.keys(excelRow).forEach(excelHeader => {
//     const backendField = COLUMN_HEADER_MAP[excelHeader] || excelHeader;
//     mappedRow[backendField] = excelRow[excelHeader];
//   });
//   return mappedRow;
// };
...
//                 const threeDecimals = ['GoldWt', 'WastageWeight', 'GNetWt', 'PTWt', 'PNetWt'];
...
//               <button onClick={() => onRowClick(row)} className="px-2 py-1 bg-indigo-600 text-white rounded flex items-center gap-2">
...
//               <td className="p-2 border"><input value={d.NoOfStones || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={d.Carat || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={d.Rate || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={d.Value || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={d.Weight || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//             </tr>
//           ))
//         }
//       </tbody>
...
//               <td className="p-2 border"><input value={c.NoOfStones || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={c.Carat || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={c.Rate || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={c.Value || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//               <td className="p-2 border"><input value={c.Weight || ''} disabled className="w-full p-1 border rounded bg-gray-100" /></td>
//             </tr>
//           ))
//         }
//       </tbody>
...
//         <h3 className="text-lg font-bold">Edit Details — Entry ID: {entry.id} | {entry.DesignNo}</h3>
...
//             </div>
//             <div>
//               <label className="text-xs text-muted-foreground">Design No</label>
...
//               <input value={entry.GrandTotal ?? ''} disabled className="w-full border p-1 rounded bg-gray-100" />
//             </div>
//             <div>
//               <label className="text-xs text-muted-foreground">Gold Weight</label>
//               <input value={entry.GoldWt ?? ''} disabled className="w-full border p-1 rounded bg-gray-100" />
//             </div>
//           </div>
//         </section>
...
//   fetchSuppliers();
// }, [ user]);
//   // Generate Excel template - STANDARD (With Gold) - Using Display Names
//   const generateStandardTemplate = () => {
//     const wb = XLSX.utils.book_new();
    
//     // Create template row using DISPLAY NAMES with placeholder values
//     const mainTemplate = [{
//       'Sno': '',
//       'Supplier Name': '',
//       'Invoice Number': '',
...
//       'Gold Purity': '',
//       'Gold Purity Wt': '',
//       'Gold 999 Rate (without GST)': '',
//       'Gold Value': '',
//       'PT Wt': '',
//       'PT Purity': '',
//       'PT Purity Wt': '',
//       'PT 999 Rate (without GST)': '',
//       'PT Value': '',
//       'No of Stones': '',
//       'Diamond Carat': '',
//       'Diamond Rate': '',
...
//     const ws1 = XLSX.utils.json_to_sheet(mainTemplate);
//     const ws2 = XLSX.utils.json_to_sheet(diamondTemplate);
//     const ws3 = XLSX.utils.json_to_sheet(csTemplate);

//     ws1['!cols'] = Array(Object.keys(mainTemplate[0]).length).fill({ wch: 15 });
...
//   // Generate Excel template - PLATINUM ONLY (No Gold) - Using Display Names
//   const generatePlatinumTemplate = () => {
//     const wb = XLSX.utils.book_new();
    
//     // Create template excluding gold columns
//     const platinumTemplate = {};
    
//     // Add only non-gold columns
//     Object.keys(COLUMN_HEADER_MAP).forEach(displayName => {
//       if (!GOLD_COLUMNS.includes(displayName)) {
//         // Add placeholder values for specific fields
//         if (displayName === 'Supplier Code') platinumTemplate[displayName] = '';
//         else if (displayName === 'Metal Type') platinumTemplate[displayName] = '';
//         else if (displayName === 'Weight Mode') platinumTemplate[displayName] = '';
//         else if (displayName === 'PT Purity Wt') platinumTemplate[displayName] = '';
//         else if (displayName === 'PT999Rate (without GST)') platinumTemplate[displayName] = '';
//         else platinumTemplate[displayName] = '';
//       }
//     });

//     const mainTemplate = [platinumTemplate];
...
//       if (isEmpty) return;
//       if (!row.id && !row.SupplierName && !row.DesignNo) {
//         errors.push(`Purchase Entry Row ${idx + 2}: provide at least id or DesignNo`);
//       }
//       if (!row.SupplierName) errors.push(`Purchase Entry Row ${idx + 2}: SupplierName is required`);
//       if (!row.ProductName) errors.push(`Purchase Entry Row ${idx + 2}: ProductName is required`);
//       if (!row.DesignNo) errors.push(`Purchase Entry Row ${idx + 2}: DesignNo is required`);
//     });

//     const purchaseIdSet = new Set();
//     mainData.forEach((row) => { if (row.id || row.id === 0) purchaseIdSet.add(String(row.id)); });

//     diamondData.forEach((row, idx) => {
//       const isEmpty = Object.values(row).every(v => v === '' || v === null || v === undefined);
//       if (isEmpty) return;
//       if (!row.purchaseEntryId) errors.push(`Diamond Detail Row ${idx + 2}: purchaseEntryId required`);
//       if (row.purchaseEntryId && !purchaseIdSet.has(String(row.purchaseEntryId))) {
//         errors.push(`Diamond Detail Row ${idx + 2}: purchaseEntryId '${row.purchaseEntryId}' not found`);
//       }
//     });
...
//       const isEmpty = Object.values(row).every(v => v === '' || v === null || v === undefined);
//       if (isEmpty) return;
//       if (!row.purchaseEntryId) errors.push(`Color Stone Row ${idx + 2}: purchaseEntryId required`);
//       if (row.purchaseEntryId && !purchaseIdSet.has(String(row.purchaseEntryId))) {
...
//         const mainDataRaw = XLSX.utils.sheet_to_json(mainSheet, { defval: '' });
...
//         // Map Excel headers (Display Names) to backend field names
//         const mainData = mainDataRaw.map(mapExcelHeadersToBackend);
        
//         // For diamond and color stone, handle both 'Sno' and 'id'
...
//           setUploadStatus({ type: 'error', message: `Validation failed (${errors.length})` });
//           setUploadedData(null);
//           return;
//         }

//         const grouped = mainData
//           .filter(r => !(Object.values(r).every(v => v === '' || v === null || v === undefined)))
//           .map(entry => {
//             const idStr = String(entry.id ?? entry.id);
//             const diamondsFor = diamondData.filter(d => String(d.purchaseEntryId) === idStr);
//             const csFor = csData.filter(c => String(c.purchaseEntryId) === idStr);
...
//         setData(grouped);
//         setValidationErrors([]);
//         setUploadStatus({ type: 'success', message: `Loaded ${grouped.length} purchase entries` });
...
//       resetForm();
//     } catch (error) {
//       setUploadStatus({ type: 'error', message: 'Error saving data!' });
//     } finally {
//       setLoading(false);
//     }
//   };
...
//     (async () => {
//       try {
//         const res = await axios.get(`${DIA_API}/supplierNames`);
...
//         {uploadStatus && (
//           <div className={`p-3 rounded ${uploadStatus.type === 'error' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
//             {uploadStatus.type === 'success' ? <CheckCircle className="inline-block mr-2" /> : <AlertCircle className="inline-block mr-2" />}
...
//               !data.length || loading ? 'bg-gray-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 text-white'
//             }`}
//           >
...
// // Column Header Mapping: Display Name -> Backend Field Name (REMOVED Supplier Name, Supplier Code, Invoice Number)
// const COLUMN_HEADER_MAP = {
//   'Sno': 'id',
//   'Product Name': 'ProductName',
//   'Metal Type': 'MetalType',
...
// // Purchase entry columns for display (backend field names)
// const PURCHASE_COLUMNS = [
//   'id', 'SupplierName', 'invoiceNumber', 'suppCode', 'ProductName', 'MetalType', 'WtMode', 'DesignNo', 'GCarat', 'PCS', 'GoldWt', 'GoldPurity', 
//   'GoldPurityWt', 'Gold999Rate', 'GoldValue', 'PTWt', 'PTPurity', 'PTPurityWt', 'PT999Rate', 'PTValue',
//   'NoofStone', 'DCarat', 'DiamondRate', 'DiamondValue', 'DiamondWt',
//   'ClrStnPCS', 'CLSCarat', 'ClrStnWt', 'ClrStnRate', 'CSAmount', 'GoMcType', 'GoMcRate', 'GoMcAmount', 
//   'PLTMcType', 'PLTMcRate', 'PLTMcAmount', 'WastageType', 'WastageWeight', 'WastageAmt','PTWastageType','PTWastageWeight','PTWastageAmt',
//   'CertType', 'CertGST', 'CertQty', 'CertRate', 'CertTaxableAmt','certTaxAmt', 'CertTotal',
//   'HallMarkType', 'HMGST', 'HMQty', 'HMRate', 'HMTaxableAmt','HmTaxAmt', 'HMTotal', 'HandleRate', 'HandleAmount',
...
//   // Generate Excel template - STANDARD (With Gold) - Common Fields at Top
//   const generateStandardTemplate = () => {
//     const wb = XLSX.utils.book_new();
    
//     // Create worksheet manually to add common fields at top
...
//       // Header row for purchase entries (without Supplier Name, Supplier Code, Invoice Number)
...
//     ws1['!merges'] = [
//       { s: { r: 0, c: 0 }, e: { r: 0, c: 5 } } // Merge first row for header
//     ];

//     // Style the common fields section
//     ws1['A1'].s = {
...
//   // Generate Excel template - PLATINUM ONLY - Common Fields at Top
//   const generatePlatinumTemplate = () => {
//     const wb = XLSX.utils.book_new();
    
//     // Get columns excluding gold columns
//     const platinumColumns = Object.keys(COLUMN_HEADER_MAP).filter(col => !GOLD_COLUMNS.includes(col));
...
//       ['Invoice Number:', ''],
//       [], // Empty row for spacing
//       // Header row for purchase entries (platinum columns only)
//       platinumColumns
//     ]);
...
//       if (!row.SupplierName) errors.push(`Purchase Entry Row ${idx + 7}: SupplierName is required (check common field)`);
//       if (!row.ProductName) errors.push(`Purchase Entry Row ${idx + 7}: ProductName is required`);
//       if (!row.DesignNo) errors.push(`Purchase Entry Row ${idx + 7}: DesignNo is required`);
//     });
...
//       const isEmpty = Object.values(row).every(v => v === '' || v === null || v === undefined);
//       if (isEmpty) return;
//       if (!row.purchaseEntryId) errors.push(`Diamond Detail Row ${idx + 2}: Purchase Entry Id required`);
//       if (row.purchaseEntryId && !purchaseIdSet.has(String(row.purchaseEntryId))) {
//         errors.push(`Diamond Detail Row ${idx + 2}: purchaseEntryId '${row.purchaseEntryId}' not found`);
//       }
//     });
...
//       const isEmpty = Object.values(row).every(v => v === '' || v === null || v === undefined);
//       if (isEmpty) return;
//       if (!row.purchaseEntryId) errors.push(`Color Stone Row ${idx + 2}: Purchase Entry Id required`);
//       if (row.purchaseEntryId && !purchaseIdSet.has(String(row.purchaseEntryId))) {
//         errors.push(`Color Stone Row ${idx + 2}: purchaseEntryId '${row.purchaseEntryId}' not found`);
//       }
//     });
...
//   // Handle file upload with common fields extraction
//   const handleFileUpload = (e) => {
//     const file = e.target.files?.[0];
//     if (!file) return;
...
//         // Extract common fields from specific cells (B2, B3, B4)
//         const supplierName = mainSheet['B2'] ? mainSheet['B2'].v : '';
//         const supplierCode = mainSheet['B3'] ? mainSheet['B3'].v : '';
//         const invoiceNumber = mainSheet['B4'] ? mainSheet['B4'].v : '';

//         // Read data starting from row 6 (after common fields section)
//         const mainDataRaw = XLSX.utils.sheet_to_json(mainSheet, { 
//           range: 5, // Start from row 6 (0-indexed as 5)
//           defval: '' 
//         });
...
//         // Map Excel headers to backend field names and add common fields
//         const mainData = mainDataRaw.map(row => {
//           const mapped = mapExcelHeadersToBackend(row);
...
//             if (key === 'Sno') {
//               mapped.id = row[key];
//             } else if (key === 'Purchase Entry Id') {
//               mapped.purchaseEntryId = row[key];
//             } else if (key === 'Stone From') {
//               mapped.STONE_FROM = row[key];
//             } else if (key === 'Diamond Shape') {
//               mapped.DiamondShape = row[key];
//             } else if (key === 'No Of Stones') {
//               mapped.NoOfStones = row[key];
//             } else {
//               mapped[key] = row[key];
//             }
//           });
...
//   'Gold Wastage Weight', 'Gold Wastage Amount', 'Gold HallMark Type',
//   'Gold HallMark GST', 'GoldHallMark Qty', 'Gold HallMark Rate',
//   'Gold HallMark Taxable Amount', 'Gold HallMark Tax Amount', 'Gold HallMark Total', 'Gold Net Wt'
// ];
...
//             </div>
//             <div>
//               <label className="text-xs text-muted-foreground">IGI Summary No</label>
//               <input value={entry.IgiSummaryNo || ''} disabled className="w-full border p-1 rounded bg-gray-100" />
//             </div>
//             <div>
//               <label className="text-xs text-muted-foreground">PGI UIN No</label>
...
// // Helper: Get default value based on data type
...
//       const isEmpty = Object.values(row).every(v => v === '' || v === null || v === undefined || v === 0);
//       if (isEmpty) return;
//       if (!row.purchaseEntryId && row.purchaseEntryId !== 0) errors.push(`Diamond Row ${idx + 2}: Purchase Entry Id required`);
...
//       const isEmpty = Object.values(row).every(v => v === '' || v === null || v === undefined || v === 0);
//       if (isEmpty) return;
//       if (!row.purchaseEntryId && row.purchaseEntryId !== 0) errors.push(`Color Stone Row ${idx + 2}: Purchase Entry Id required`);
//       if ((row.purchaseEntryId || row.purchaseEntryId === 0) && !purchaseIdSet.has(String(row.purchaseEntryId))) {
//         errors.push(`Color Stone Row ${idx + 2}: purchaseEntryId '${row.purchaseEntryId}' not found`);
//       }
//     });
...
//           Object.keys(row).forEach(key => {
//             if (key === 'Sno') {
//               mapped.id = getDefaultValue('id', row[key]);
//             } else if (key === 'Purchase Entry Id') {
//               mapped.purchaseEntryId = getDefaultValue('id', row[key]);
//             } else if (key === 'Stone From') {
//               mapped.STONE_FROM = getDefaultValue('STONE_FROM', row[key]);
//             } else if (key === 'Diamond Shape') {
//               mapped.DiamondShape = getDefaultValue('DiamondShape', row[key]);
//             } else if (key === 'No Of Stones') {
//               mapped.NoOfStones = getDefaultValue('NoOfStones', row[key]);
//             } else if (key === 'Carat') {
//               mapped.Carat = getDefaultValue('Carat', row[key]);
//             } else if (key === 'Rate') {
//               mapped.Rate = getDefaultValue('Rate', row[key]);
//             } else if (key === 'Value') {
//               mapped.Value = getDefaultValue('Value', row[key]);
//             } else if (key === 'Weight') {
//               mapped.Weight = getDefaultValue('Weight', row[key]);
//             } else {
//               mapped[key] = row[key];
//             }
//           });
...
//               mapped.purchaseEntryId = getDefaultValue('id', row[key]);
//             } else if (key === 'Color Stone Shape') {
//               mapped.csShape = getDefaultValue('csShape', row[key]);
//             } else if (key === 'No Of Stones') {
//               mapped.NoOfStones = getDefaultValue('NoOfStones', row[key]);
//             } else if (key === 'Carat') {
//               mapped.Carat = getDefaultValue('Carat', row[key]);
...
//           .map(entry => {
//             const idStr = String(entry.id ?? entry.id);
//             const diamondsFor = diamondData.filter(d => String(d.purchaseEntryId) === idStr && Object.values(d).some(v => v !== '' && v !== 0));
//             const csFor = csData.filter(c => String(c.purchaseEntryId) === idStr && Object.values(c).some(v => v !== '' && v !== 0));
//             return { ...entry, diamonds: diamondsFor, colorStones: csFor };
//           });
