...
const ReturnDialog = ({ isOpen, onClose, onSubmit, rowData }) => {
...
const PurchaseReturn = () => {
    ...
    const fetchSuppliers = async () => {
      try {
        const response = await axios.get(
          `${DIA_API}/purchase_return/supplierNames`
        );
  ...
  useEffect(() => {
    const fetchInvoiceNumber = async () => {
      try {
        const response = await axios.get(
          `${DIA_API}/purchase_return/supplierNames/${selectedSupplier}`
        );
  ...
  useEffect(() => {
    fetchData();
  }, [selectedSupplier, selectedInvoice]);
  const fetchData = async () => {
    if (!selectedSupplier) return;
    try {
      const response = await axios.get(
        `${DIA_API}/purchase_return/SupplierDetails/${selectedSupplier}/${btoa(
          selectedInvoice
        )}`
      );
      console.log(
  ...
  const handleReturn = async (formData, images, remarks) => {
    ...
    try {
      ...
      if (selectedSupplier) {
        const response = await axios.get(
          `${DIA_API}/purchase_return/SupplierDetails/${selectedSupplier}/${selectedInvoice}`
        );
        setData(response.data);
      }
      ...
      <div className="mb-6 flex justify-between items-center">
        ...
        <div className="flex gap-3">
          {selectedRows.size > 0 && (
            ...
            >
              Return Selected ({selectedRows.size})
            </button>
          )}
          <button
          ...
          <table className="min-w-full bg-white border">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="border p-2 text-left">Upload Remarks</th>
              </tr>
            </thead>
            <tbody>
              {data.map((row, index) => (
